...
At the bottom, a centred `.empty-cta` container holds the `#btn-see-projection` primary button (navy fill, paper text, 12×28 padding). Clicking it sets `projectionRequested = true`, calls `renderEvents()` to freshen the drawer, and calls `refresh()`, which re-runs `deriveViewState()` and flips the canvas to State 2. The button is **disabled until both spouse ages parse as finite integers** (Session 11) — in the disabled state it renders at 40% opacity with a `not-allowed` cursor, enforced by `.empty-cta .btn.primary:disabled { opacity: 0.4; cursor: not-allowed; }`. Retirement age, balances, and contributions can stay blank when the adviser fires the projection; the calculator reads blank money fields as 0 and blank retirement age as the internal fallback 65 (invisible, no pre-filled default in the input). Hidden in print via `.empty-cta` in the `@media print` hide list.
