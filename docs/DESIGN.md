...
Sits between the outcome strip and the narrative, reusing the `.narrative` shell (same card surface, same 2px gold left-rule) so it reads as a sibling of "In plain terms" rather than a new visual idiom. Its job is to make the income goal *actionable*.
