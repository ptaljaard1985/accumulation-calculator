...
At the bottom, a centred `.empty-cta` container holds the `#btn-see-projection` primary button (navy fill, paper text, 12×28 padding). Clicking it sets `projectionRequested = true`, calls `renderEvents()` to freshen the drawer, and calls `refresh()`, which re-runs `deriveViewState()` and flips the canvas to State 2. Hidden in print via `.empty-cta` in the `@media print` hide list.
