...
- `capitalEventDisplayRows(events)` / `capitalEventsHtml(display, rowClass, emptyClass, moreClass)` — *(v2, Session 23)* shared display adapters living next to `escapeHtml`, used by both the screen recap card and the report Page-2 strip. `capitalEventDisplayRows` returns `{ rows, extraCount, hasEvents }`, capping at the first 3 events (`row = { age:'Age N', kind:'Inflow'|'Outflow', amount:fmtR(amount), basis:"Today's money"|'Future rands' }`); `capitalEventsHtml` renders them to HTML (kind class `event-kind-inflow|outflow`, an "N additional event(s) modelled." line when `extraCount > 0`, `escapeHtml` on every value). Formatting only — no math.
