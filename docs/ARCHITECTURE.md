...
`newEvent()` produces a default (inflow, `name:''`, age = reference + 10, R500k PV). Each event is `{ id, name, age, amount, todaysMoney, kind }`; `name` is optional free text (max 20 chars, Session 24). `renderEvents()` paints the editor markup — a 5-column grid (name · kind · age · amount+basis+year · delete). It iterates the EDITOR container IDs `['empty-events-list', 'modal-events-list']` (State 1 full-width inline editor + the roomy modal), painting the same rows into both from the same `eventsStore`; event delegation is attached once per container (guarded by `dataset.wired`). After the editor loop it also paints the drawer **read-only** summary `#drawer-events-summary` via the shared `capitalEventsHtml(capitalEventDisplayRows(eventsStore), …)`. `readEvents()` returns the sanitised list, now including `name` (so it reaches `p.inputs.events`; `project()` ignores it).

**Drawer entry is via a modal (Session 24).** The drawer column II no longer holds an inline editor — it shows the `#drawer-events-summary` + a "Manage capital events" button (`#drawer-manage-events-btn`) that opens `#events-modal` (cloned from the `#report-scenario-modal` pattern — `.report-modal-backdrop.events-modal-backdrop`, `width: min(820px, 100%)`, `data-open` toggle via `openEventsModal`/`closeEventsModal`, dismissed by backdrop click or the Done button). The modal hosts `#modal-events-list` + `#modal-add-event-btn`; **Done** calls `renderEvents()` (to repaint the drawer summary with any name edits) then `closeEventsModal()`. The State-1 `.empty-events-panel` keeps its full-width inline editor (`#empty-events-list` + `#empty-add-event-btn`).

Cross-container sync: `onEventInput` updates the year-label inline in the container the user is typing in (preserving focus) and handles the `name` field (slice-capped at 20). When the adviser clicks the `#btn-see-projection` CTA, the handler calls `renderEvents()` before `refresh()`, so the drawer summary picks up any events added/edited in State 1.
...
- `updateRecapCard(p)` — *(v2 only)* paints the "Current plan" recap card from `p.inputs` (balances, contributions, return/CPI/escalation) via `fmtR` / `fmtPct` and the null-safe `set()`. Called right after `updatePlanBar(p)`. Its spouse-name column headers carry `data-spouse` + `data-spouse-template="header"` so `renderSpouseLabels()` syncs them. Ids: `recap-ret-A/B`, `recap-disc-A/B`, `recap-contrib-ret-A/B`, `recap-contrib-disc-A/B`, `recap-return`, `recap-cpi`, `recap-esc`. **Session 23:** it also fills `#recap-events-list` from `p.inputs.events` via the shared `capitalEventsHtml(capitalEventDisplayRows(...))` helper (empty → "No capital events modelled.").
- `capitalEventDisplayRows(events)` / `capitalEventsHtml(display, rowClass, emptyClass, moreClass)` — *(v2, Session 23)* shared display adapters living next to `escapeHtml`, used by the screen recap card, the **drawer summary** (`#drawer-events-summary`, Session 24), and the report Page-2 strip. `capitalEventDisplayRows` returns `{ rows, extraCount, hasEvents }`, capping at the first 3 events (`row = { name:'…' (≤20 chars), age:'Age N', kind:'Inflow'|'Outflow', amount:fmtR(amount), basis:"Today's money"|'Future rands' }`); `capitalEventsHtml` renders them to HTML — the kind cell is an `.event-label` span holding `<name> (Inflow)` when named (name in `.event-name`, kind in `.event-kind event-kind-inflow|outflow`) or the bare `Inflow`/`Outflow` (no parens) when unnamed, plus an "N additional event(s) modelled." line when `extraCount > 0`, `escapeHtml` on every value. Formatting only — no math.
...
- `buildReportData(p, baseline, events, names)` — **pure formatting adapter, no math.** Maps live state to report slots: names (spouse first names, family-name fallback via `deriveFamilyName(#client-name)` when both are defaults), cover income, the 4 assumptions, per-spouse household figures, chart points from `incomeCurveData(p.inputs)`, methodology prose with live assumptions, and (when `baseline`) the scenario comparison. **Session 23:** also returns `capitalEvents: capitalEventDisplayRows(p.inputs.events)` (so the Page-2 strip follows the baseline when one is locked — `p` is the baseline projection there) and makes the methodology events sentence conditional on `p.inputs.events.length` (year-end-discretionary paragraph vs "No capital events are modelled in this projection.").
...
- `populateReportDeck(d)` — writes every `[data-bind="…"]` slot via `setBind`/`setBindText`, builds the scenario table rows, and calls `renderReportIncomeChart`. **Session 23:** toggles the Page-2 capital-events strip (`#report-capital-events` display + a `has-events` class on `.projection-page-body` that swaps the grid `108mm 1fr` → `92mm minmax(0,1fr) auto`) on `d.capitalEvents.hasEvents`, then binds `report-events-list` via `capitalEventsHtml`. When there are no events the strip is hidden and the grid is unchanged, so the existing 3/4-page output is byte-identical.
