...
**Follow-up fix (after PR #35 merged):** shrinking the projection chart panel 80mm → 64mm re-introduced the side letterboxing the income chart had previously been tuned to avoid. Cause: the income-chart SVG (`#rr-income-chart`, `renderReviewIncomeChart`) had a fixed `viewBox 0 0 1460 350` (4.17:1) with the default `preserveAspectRatio`; at 80mm the panel was ~4.2:1 so the width was the limiting scale (fills width, tiny top/bottom letterbox), but at 64mm the panel became ~5.56:1 so the *height* became limiting → ~34mm of empty navy on each side. Fix: shortened the viewBox + drawing height to `1460×260` (5.6:1, just wider than the 5.56:1 panel) so the width is limiting again and the chart fills the panel. Units render at the same physical size (the width scale `269mm/1460` is unchanged), so the Early/Late/main callout boxes are unaffected — only the vertical drawing area is more compact. Verified: 3 callouts, all in-bounds of the 260-tall viewBox, zero overlaps.
...
- **The page** (`data-page="scenario"`, `data-enabled="false"` in markup, inserted between projection and risk, footer `06 / 09` → renumbered live). `.rr-scenario-page`: an overlaid two-curve income chart (`#rr-scenario-chart`, viewBox `0 0 1460 260` — shortened this session so the panel fits the page, still no letterboxing), a two-tile hero (Current plan vs Scenario, with a signed income delta), and a full-width **"What changed" card** holding **six levers in two fixed three-row columns** (`.rr-scenario-cols`, tbodies `#rr-scenario-changes` / `#rr-scenario-changes-b`): the client's own decisions on the left (retirement contributions, discretionary contributions, retirement age), the assumptions on the right (expected return, assumed inflation, contribution escalation). Income goal and the derived goal-progress row are omitted (the hero carries the income outcome). `populateReviewReport` selects each column by label from `sc.cmp.rows` (via a `scByLabel` map + `scPick`), so the two reports still share `buildScenarioComparison` unchanged.
- **Layout fix (same session, follow-up to the first print).** The first print overflowed onto a 2nd sheet: the single 8-row lever table + a right-hand **Notes** box exceeded the ~168mm content budget (A4 landscape 297×210mm, less the 19mm header, 12mm footer, and 11mm body padding). Four changes, each verified against the budget by an independent multi-agent check (three budget re-derivations agreed the page overflowed by ~4.5–8.5mm before these, and clears by ~10mm after):
  1. **Notes box removed**, detail card made full-width (its `resultNote`/`changedNote` are still computed by `buildScenarioComparison` for the landscape deck, just unused here — the hero already shows the income delta).
  2. **Levers split into two columns** (tbodies `#rr-scenario-changes` / `-b`). Removing Notes freed *horizontal* space, not vertical; the column split is what halves the table height. (Later reduced from 8 levers/4-per-column to **6 levers/3-per-column** — see below.)
  3. **Scenario chart shortened** (viewBox 320 → 260; the SVG self-sizes to its aspect ratio across the full panel width, so it was the single biggest vertical consumer at ~76mm).
  4. **Padding cascade won.** The scenario table's `td`/`th` padding was being overridden by the later, equal-specificity generic `.rr-table td { padding: 7px 0 }` — so rows were 7px tall with 0 horizontal cell padding. Bumped the scenario rules to `table.rr-scenario-table td/th` (one element selector higher) so the intended `4px 8px` wins: shorter rows *and* proper horizontal spacing. Hero tile padding and the page grid gap were also trimmed.
- **Changed levers are highlighted in the report gold.** `renderReviewScenarioRows` tags a `<tr>` with `rr-row-changed` when `r.d.c !== 'delta-neutral' && r.input` (only real *levers* — the derived Goal-progress row is excluded), and `.rr-scenario-table tr.rr-row-changed td { color: var(--rr-gold) }` colours the whole row gold (specificity `(0,3,2)` beats the per-cell green/red change rule's `(0,3,0)`, so it wins on the change cell too). Unchanged levers stay muted.
...
- **`populateReviewReport(includeScenario)`** — computes `showScenario = !!(includeScenario && baseline && lastProjection)`, toggles the page's `data-enabled`, and when shown fills the hero (`scen-current`/`scen-plan`/`scen-delta` + the delta's colour class), **selects the six kept levers into two fixed columns by label** (`scByLabel` map + `scPick([...])` — left = client decisions, right = assumptions; income goal + goal progress omitted), and renders the chart. `renumberReviewPages()` filters out `data-enabled="false"` pages before stamping `NN / TT`, so the report is 9 pages without a scenario and 10 with one. A print-block rule hides disabled pages under headless `--print-to-pdf` too.
- **Two review-report page titles renamed** (client-facing wording): the risk page h1 `Policy summary.` → **`Risk Planning Summary`**, the estate page h1 `Estate facts to confirm.` → **`Estate Planning Summary`** (kickers unchanged). No test pinned the old strings.

**Decisions (and why):** reuse `buildScenarioComparison` (pure data, already tested and em-dash-free) rather than a bespoke scenario form — the levers the advisor already knows are the input surface, and the comparison logic is shared with the landscape deck so the two reports can't drift. Baseline = "current plan", live plan = "scenario" matches how the advisor thinks in the meeting (lock what we have, then explore). Chart viewBox is 1460×260 (wide enough to fill the panel with no side letterbox; shortened from 320 during the layout fix below so the page fits one sheet).

**Tests:** 92 JS (88 + 4: page-count now 10 = 9 always-on + 1 conditional scenario; button wired to `startReviewReport`; scenario page markup + `data-enabled="false"` + `rr-scenario-chart`/`rr-scenario-changes`; `renumberReviewPages` skips disabled; `startReviewReport` gates on baseline; `buildReviewScenarioData` reuses `buildScenarioComparison(lastProjection, baseline)` + both `incomeCurveData` calls; `populateReviewReport` requires `includeScenario && baseline && lastProjection`; the scenario test extended for the two-column layout (`rr-scenario-changes-b` + `rr-scenario-cols`), the removed Notes box, the gold `rr-row-changed` rule, the fixed `scLeft`/`scRight` label selection, and that income goal + goal progress are dropped) + 60 Python (unchanged — no math change), all green. Regenerated `Meeting Report/review-report-preview.html` (git-ignored) with a sample tweak (+R5 000/mo retirement, retire at 67, return 5%→6%): six levers in two three-row columns (client decisions left, assumptions right), changed levers (retirement contributions, retirement age, expected return) in gold, overlaid two-curve chart, page 06 / 10.

**Known caveat:** No headless browser this session. The one-page fit was engineered against a computed vertical budget and cross-checked by an independent multi-agent estimate (~10mm margin after the four layout changes above), but the exact rendering still wants a final browser print-preview — the residual uncertainty is how Chrome resolves the SVG height, which the shortened viewBox de-risks. The no-scenario path is unchanged, so the existing 9-page output is safe.
