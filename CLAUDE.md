...
- **Layout fix (same session, follow-up to the first print).** The first print overflowed onto a 2nd sheet: the single 8-row lever table + a right-hand **Notes** box exceeded the ~168mm content budget (A4 landscape 297×210mm, less the 19mm header, 12mm footer, and 11mm body padding). Four changes, each verified against the budget by an independent multi-agent check (three budget re-derivations agreed the page overflowed by ~4.5–8.5mm before these, and clears by ~10mm after):
  ...
  2. **Levers split into two 4-row columns** (`scHalf = Math.ceil(rows.length/2)`, tbodies `#rr-scenario-changes` / `-b`). Removing Notes freed *horizontal* space, not vertical; the column split is what halves the table height.
...
- **`populateReviewReport(includeScenario)`** — computes `showScenario = !!(includeScenario && baseline && lastProjection)`, toggles the page's `data-enabled`, and when shown fills the hero (`scen-current`/`scen-plan`/`scen-delta` + the delta's colour class), **splits the lever rows across the two columns** (`slice(0, scHalf)` / `slice(scHalf)`), and renders the chart. `renumberReviewPages()` filters out `data-enabled="false"` pages before stamping `NN / TT`, so the report is 9 pages without a scenario and 10 with one. A print-block rule hides disabled pages under headless `--print-to-pdf` too.
